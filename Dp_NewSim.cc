...
*/
  ...
  double d = 0.164/pow((params->AA),2./3.);
  double ap = 773.0/Mmu/pow((params->ZZ),2./3.);
  double a = 111.0/Mmu/pow((params->ZZ),1./3.);
  double G2el = (params->ZZ)*(params->ZZ)*a*a*a*a*t*t/(1.0+a*a*t)/(1.0+a*a*t)/(1.0+t/d)/(1.0+t/d);
  double G2in = (params->ZZ)*ap*ap*ap*ap*t*t/(1.0+ap*ap*t)/(1.0+ap*ap*t)/(1.0+t/0.71)/(1.0+t/0.71)
    /(1.0+t/0.71)/(1.0+t/0.71)/(1.0+t/0.71)/(1.0+t/0.71)/(1.0+t/0.71)/(1.0+t/0.71)
    *(1.0+t*(MUp*MUp-1.0)/4.0/Mpr/Mpr)*(1.0+t*(MUp*MUp-1.0)/4.0/Mpr/Mpr);
...
//   double ThetaMaxA = pow(MA/E0,1.5);
   ...
   double integratedXmin = -log(MA*MA*(1.-Xmin))/MA/MA;
...
//   srand (time(NULL));
   ...
   for( int iii = 1; iii < 10000; iii++) 
   {
      ...
      double A  = drand48() * (integratedX-integratedXmin)+integratedXmin;
...
momentum DarkPhotons::MuSimulateEmission(double E0)
{
   momentum fParticle;
   double Xmin = MA/E0;
   double Xmax = 1.;
   if(Mmu>MA) Xmax = 1-Mmu/E0;
   else Xmax = 1-MA/E0;
   double alpha = MA/(Mmu+MA);
   double beta = (Mmu/(Mmu+MA))*(Mmu/(Mmu+MA));
   double integratedX = log(alpha*(1-Xmax)+beta*Xmax)/(beta-alpha);
   double integratedXmin = log(alpha*(1-Xmin)+beta*Xmin)/(beta-alpha);
...
//   double integratedX = -log(MA*MA*0.05*(1-Xmax))/MA/MA*20.;
   ...
   {
      double A  = drand48() * (integratedX-integratedXmin)+integratedXmin;
      double XEv = (exp(A*(beta-alpha))-alpha)/(beta-alpha);
      double smax = 1./(MA*MA*(alpha*(1.-XEv)+beta*XEv));
      double UU = drand48() * smax;  
      double sigma = (1.-XEv+XEv*XEv/3.)/(MA*MA*(1-XEv)/XEv+Mmu*Mmu*XEv);
      if(sigma > smax)  printf ("Maximum violated: ratio = % .18f, X: %e", sigma/smax, XEv);

      if(sigma >= UU) 
      {
	 XAcc = (1-XEv)*(E0-Mmu-MA)+Mmu;
         ...
         for (int j=0;j<100;j++)
	 {
            ...
            if (Pt*Pt + Mmu*Mmu < XAcc*XAcc)
	    {
	       double P = sqrt(XAcc*XAcc-Mmu*Mmu);
               fParticle.E0 = XAcc;
               fParticle.Theta = asin(Pt/P);
               fParticle.Phi = drand48()*2*3.14159;
